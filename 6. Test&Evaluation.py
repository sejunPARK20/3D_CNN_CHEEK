...
with torch.no_grad():
    for batch_x, batch_y in test_loader:
        batch_x = batch_x.to(device)  # (B, 4, T, 64, 64)
        outputs = model(batch_x)        # (B, T), 정규화된 출력
        all_preds.append(outputs.cpu())
        all_truths.append(batch_y)

all_preds = torch.cat(all_preds, dim=0)   # (N, T)
all_truths = torch.cat(all_truths, dim=0) # (N, T)
...
for i in range(num_clips_to_plot):
    true_ppg = all_truths[i].numpy()   # (T,)
    pred_ppg = preds_denorm[i].numpy()  # (T,)
...
plt.ylabel("PPG")
plt.title(f"OnlyCheek_700_Samples - <test{test_num}>")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
